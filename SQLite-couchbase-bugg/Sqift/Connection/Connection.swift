...
class Connection {
    ...
    var fileName: String { return String(cString: sqlite3_db_filename(handle, nil)) }
    ...
    var threadSafe: Bool { return sqlite3_threadsafe() > 0 }
    ...
    var lastInsertRowID: Int64 { return sqlite3_last_insert_rowid(handle) }
    ...
    var changes: Int { return Int(sqlite3_changes(handle)) }
    ...
    var totalChanges: Int { return Int(sqlite3_total_changes(handle)) }
    ...
    /// Executes the SQL statement in a single-step by calling prepare, step and finalize.
    ///
    ...
    func prepare(_ sql: SQL, _ parameters: Bindable?...) throws -> Statement {
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters an error compiling the SQL statement or binding the parameters.
    func prepare(_ sql: SQL, _ parameters: [String: Bindable?]) throws -> Statement {
        let statement = try Statement(connection: self, sql: sql)
        if !parameters.isEmpty { try statement.bind(parameters) }
    ...
    /// Runs the SQL statement in a single-step by calling prepare, bind, step and finalize.
    ///
    ...
    func run(_ sql: SQL, _ parameters: Bindable?...) throws {
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters and error when running the SQL statement.
    func run(_ sql: SQL, _ parameters: [String: Bindable?]) throws {
        try prepare(sql).bind(parameters).run()
    }
    ...
    func attachDatabase(from storageLocation: StorageLocation, withName name: String) throws {
