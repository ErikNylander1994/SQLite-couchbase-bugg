...
extension Connection {
    ...
    typealias CustomBusyHandler = (_ invocationCount: Int32) -> Bool

    // MARK: - Helper Types

    /// Represents the type of busy handler to be used on the connection.
    ///
    /// - timeout:         Creates an busy handler in SQLite that sleeps for a specified amount of time when a
    ...
    func busyHandler(_ handler: BusyHandler) throws {
        ...
        case let .custom(handler):
            ...
            result = sqlite3_busy_handler(
                handle, {(boxPointer: UnsafeMutableRawPointer?, invocationCount: Int32) -> Int32 in
