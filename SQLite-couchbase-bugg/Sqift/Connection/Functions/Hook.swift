...
extension Connection {
    ...
    typealias UpdateHook = (UpdateHookType, _ databaseName: String?, _ tableName: String?, _ rowID: Int64) -> Void
    ...
    func updateHook(_ hook: UpdateHook?) {
        ...
        sqlite3_update_hook(
            handle, {(boxPointer: UnsafeMutableRawPointer?, type, databaseName, tableName, rowID) in
...
extension Connection {
    ...
    func commitHook(_ hook: CommitHook?) {
        guard let hook = hook else {
        ...
        sqlite3_commit_hook(
            handle, {(boxPointer: UnsafeMutableRawPointer?) -> Int32 in
...
extension Connection {
    ...
    func rollbackHook(_ hook: RollbackHook?) {
        guard let hook = hook else {
        ...
        sqlite3_rollback_hook(
            handle, {(boxPointer: UnsafeMutableRawPointer?) in
                guard let boxPointer = boxPointer else { return }
