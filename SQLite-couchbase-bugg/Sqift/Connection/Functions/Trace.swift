...
extension Connection {
    ...
    ///                     connection.
    enum TraceEvent: CustomStringConvertible {
        ...
        static let statementMask = UInt32(SQLITE_TRACE_STMT)
        
        /// Returns the `.profile` bitwise mask.
        static let profileMask = UInt32(SQLITE_TRACE_PROFILE)
        
        /// Returns the `.row` bitwise mask.
        static let rowMask = UInt32(SQLITE_TRACE_ROW)
        
        /// Returns the `.connectionClosed` bitwise mask.
        static let connectionClosedMask = UInt32(SQLITE_TRACE_CLOSE)
    ...
    ///   - closure: The closure called when SQLite calls step on a statement.
    func traceEvent(mask: UInt32? = nil, closure: ((TraceEvent) -> Void)?) {
        ...
        sqlite3_trace_v2(
            handle,
            mask, {(mask: UInt32, boxPointer: UnsafeMutableRawPointer?, arg1: UnsafeMutableRawPointer?, arg2: UnsafeMutableRawPointer?) in
