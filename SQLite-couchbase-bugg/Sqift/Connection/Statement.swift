...
class Statement {
    ...
    var isBusy: Bool { return sqlite3_stmt_busy(handle) != 0 }
    ...
    var isReadOnly: Bool { return sqlite3_stmt_readonly(handle) != 0 }

    /// Returns the SQL text used to create the statement.
    var sql: SQL { return SQL(cString: sqlite3_sql(handle)) }
    ...
    init(connection: Connection, sql: SQL) throws {
    ...
    @discardableResult
    func bind(_ parameters: Bindable?...) throws -> Statement {
        try bind(parameters)
        return self
    }
    ...
    /// - Throws: A `SQLiteError` if binding the parameters to the statement encounters an error.
    @discardableResult
    func bind(_ parameters: [String: Bindable?]) throws -> Statement {
        try reset()
    ...
    func query<T: Extractable>() throws -> T? {
        ...
        return try T.fromBindingValue(bindingValue) as? T
    }
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters an error stepping through the statement.
    func query<T: ExpressibleByRow>() throws -> T? {
        guard try step() else { return nil }
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters an error stepping through the statement.
    func query<T>(_ body: (Row) throws -> T) throws -> T? {
        guard try step() else { return nil }
        let row = Row(statement: self)
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters an error stepping through the statement.
    func query<Key: Hashable, Value>(_ body: (Row) throws -> (Key, Value)) throws -> [Key: Value] {
    ...
    ///
    /// - Throws: A `SQLiteError` if SQLite encounters an error stepping through the statement.
    func query<Key, Value>(_ body: ([Key: Value], Row) throws -> (Key, Value)) throws -> [Key: Value] {
        var results: [Key: Value] = [:]
    ...
    func columnIndex(forName name: String) -> Int? {
        for (index, columnName) in columnNames.enumerated() where columnName == name {
            return index
        }
