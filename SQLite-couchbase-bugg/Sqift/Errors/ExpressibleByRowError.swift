...
struct ExpressibleByRowError: Error {
    ...
    init(type: ExpressibleByRow.Type, row: Row) {
        self.type = type
...
extension ExpressibleByRowError: CustomStringConvertible {
    ...
    var description: String { return "ExpressibleByRowError: \(errorDescription ?? "nil")" }
}
...
extension ExpressibleByRowError: LocalizedError {
    ...
    var errorDescription: String? { return "Failed to initialize \(type) from Row with columns: \(columns)" }

    /// A localized message describing the reason for the failure.
    var failureReason: String? { return "\(type) could not be initialized from Row with columns: \(columns)" }
}
